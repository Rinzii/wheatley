...
const JOIN_HISTORY_PAGE_SIZE = 20;
...
export default class VoiceLog extends BotComponent {
    ...
    private build_log_message(
        ...
        const lines = newest_first.map(e => {
            const name = e.display_name || e.username || e.user_id;
            const kind = e.kind === "join" ? "🟩 **JOIN**" : "🟥 **LEAVE**";
            const profile = `[\`${e.user_id}\`](https://discord.com/users/${e.user_id})`;
            const user = `**${name}** (\`${e.username}\`) • ${profile}`;
            ...
            return `${kind} — ${user} — ${when}${move}`;
        });

        const pages = Math.ceil(lines.length / JOIN_HISTORY_PAGE_SIZE);
        const clamped_page = Math.min(Math.max(page, 0), pages - 1);
        const page_lines = lines.slice(
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .setDescription(page_lines.join("\n"))
            .setFooter({
                text: `${lines.length} event${lines.length === 1 ? "" : "s"} shown (max ${effective_n})`,
            });
